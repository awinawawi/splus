<?php
$meta['title']          = 'PVC Foam Board - S-Plus Indonesia';
$meta['description']    = 'PVC Foam Board atau pvc board adalah papan lembaran pabrikan yang diproduksi dari bahan polyvinyl chloride dengan karakteristik kedua permukaaannya yang keras sehingga menciptakan produk yang berkualitas kuat, tahan lama dan ramah lingkungan. dikarenakan kepadatan yang tinggi dan ringan ditambah produksi kayu yang semakin berkurang produk ini sangat banyak digunakan sebagai pengganti papan kayu yang ideal.';
$meta['keywords']       = 'pvc foam board, pvc board, foam board, jual pvc foam board, jual pvc board, jual foam board, pvc, pvc foam board murah, pvc foam board berkualitas';
$this->load->view('user/header', $meta) ?>
<main class="ps-main">
    <div class="ps-section--features-product ps-section masonry-root pb-40 bg-grey">
        <div class="ps-container">
            <div class="row pb-50 pt-50">
                <?php if (!$this->agent->is_mobile()) : ?>
                    <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 bg-white shd">
                        <!-- <aside class="ps-widget--sidebar ">
                            <div class="ps-widget__header">
                                <h3>KATEGORI</h3>
                            </div>
                          
                            <div class="ps-widget__content ">
                                <ul class="ps-list--arrow">
                                    <li class="panel-title" data-toggle="collapse" data-target="#test">
                                        <a href="<?= base_url('kategori/s-plus-pintu') ?>"><b>PINTU</b>
                                        </a>
                                        <ul id="test">
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-pintu/abs') ?>">PINTU ABS</a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-pintu/upvc') ?>"><b>PINTU UPVC</b></a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-pintu/namo') ?>">PINTU NAMO</a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-pintu/iroom_door') ?>">PINTU IROOM</a></li>
                                        </ul>

                                    </li>

                                    <li class="panel-title" data-toggle="collapse" data-target="#test2">
                                        <a href="<?= base_url('kategori/s-plus-upvc-jendela') ?>"><b>JENDELA</b></a>
                                        <ul id="test2">
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-upvc-jendela/jendela_upvc') ?>">JENDELA UPVC</a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-upvc-jendela/jendela_IROOM') ?>"><b>JENDELA IROOM</b></a></li>

                                        </ul>
                                    </li>

                                    <li class="panel-title" data-toggle="collapse" data-target="#test3">
                                        <a href="<?= base_url('kategori/s-plus-upvc-plafon/plafon') ?>"><b>INLIFE</b></a>
                                        <ul id="test3">
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-upvc-plafon/plafon_upvc') ?>">PLAFON UPVC</a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-sun-shade/sun-shead') ?>"><b>SUN SHADE</b></a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-solid-surface/solid') ?>"><b>SOLID SURFACE</b></a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-kitchen-set') ?>"><b>KITCHEN SET</b></a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-pvc-foam-board') ?>"><b>PVC FOAM BOARD</b></a></li>
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-vinyl-flooring/vinyl_kategori') ?>"><b>VINYL FLOORING</b></a></li>
                                        </ul>
                                    </li>

                                    <li class="panel-title " data-toggle="collapse" data-target="#test4">
                                        <a href="<?= base_url('kategori/s-plus-wpc') ?>"><b>OUTLIFE</b></a>
                                        <ul id="test4">
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-wpc/wpc_kategori') ?>">WPC</a></li>
                                            <li class="sm"><a href='https://sinargarden.com/parasol'><b>PARASOL</b></a></li>
                                        </ul>
                                    </li>

                                    <li class="panel-title " data-toggle="collapse" data-target="#test5">
                                        <a href="<?= base_url('kategori/s-plus-face-mask') ?>"><b>MASKER</b></a>
                                        <ul id="test5">
                                            <li class="sm"><a href="<?= base_url('kategori/s-plus-wpc') ?>">MASKER DEWASA</a></li>
                                            <li class="sm"><a href='https://sinargarden.com/parasol'><b>MASKER ANAK-ANAK</b></a></li>

                                        </ul>
                                    </li>
                                </ul>
                            </div>

                        </aside> -->
                    </div>
                <?php endif; ?>
                <div class="col-lg-12">
                    <div class="ps-box pb-60 pt-20 pl-20 pr-20 bg-white shd">
                        <div class="">
                            <img src="<?= base_url('assets/user/images/kategori/splus-pvc-foam-board/banner.jpg') ?>" class="pb-20">
                        </div> <br>
                        <div class="">
                            <center>
                                <span class="h-subtitle h-m-title"><strong>PVC Foam Board</strong></span>
                            </center>
                            <hr style="margin-bottom: 25px;width: 25%; border: 0.5px solid #e24545">
                            <p class="h-body h-m-body" style="text-align:justify;">
                                PVC Foam Board atau pvc board adalah papan lembaran pabrikan yang diproduksi dari bahan polyvinyl chloride dengan karakteristik kedua permukaaannya yang keras sehingga menciptakan produk yang berkualitas kuat, tahan lama dan ramah lingkungan. dikarenakan kepadatan yang tinggi dan ringan ditambah produksi kayu yang semakin berkurang produk ini sangat banyak digunakan sebagai pengganti papan kayu yang ideal.
                            </p>
                            <div class="row pb-20">
                                <div class="col-md-12 col-sm-12 col-xs-12 pt-20" style="padding-top:15px; ">
                                    <span class="h-subtitle h-m-title" style="font-size:23px">Keunggulan PVC Foam Board :</span><br>
                                    <ul class="a h-body h-m-body">
                                        <li>Padat dan Ringan</li>
                                        <li>Tahan Air, Cuaca, Panas, Bocor & Sinar UV</li>
                                        <li>Anti Rayap, Bakteri dan Jamur</li>
                                        <li>Permukaan Halus</li>
                                        <li>Ramah Lingkungan</li>
                                    </ul>
                                </div>
                            </div>
                            <!-- <div class="pt-20 text-center pb-20">
                                <span class="h-subtitle h-m-title"><strong>Spesifikasi PVC Foam Board</strong></span>
                                <hr style="margin-bottom: 5px;width: 40%; border: 0.5px solid #e24545">
                                <div class="row pt-20">
                                    <img width="<?php echo ($this->agent->is_mobile() ? "85%" : "96%") ?>" src="<?= base_url('assets/user/images/kategori/splus-pvc-foam-board/spesifikasi.jpg') ?>" alt="">
                                </div>
                            </div> -->
                        </div>
                        <div class="">
                            <center style="padding-top: 20px;">
                                <span class="h-subtitle h-m-title"><strong>Jenis-Jenis PVC Foam Board</strong></span>
                            </center>
                            <hr style="margin-bottom: 25px;width: 30%; border: 0.5px solid #e24545">
                            <div class="row pb-20">

                                <?php if ($this->agent->is_mobile()) : ?>
                                    <div class="col-sm-12 col-xs-12">
                                        <img src="<?= base_url('assets/user/images/kategori/produk/pvc/pvc-m.jpg') ?>" style="padding-top:10px;">
                                    </div>
                                <?php else : ?>
                                    <div class="col-md-12">
                                        <img src="<?= base_url('assets/user/images/kategori/produk/pvc/pvc.jpg') ?>" style="padding-top:10px;">
                                    </div>
                                <?php endif; ?>
                            </div>
                            <!-- <div class="row pb-20">
                                <h3 style="padding-left: 15px;"><b>Cabinet Bathroom</b></h3>
                                <hr style="width: 28%; border: 0.5px solid #e24545; margin-left:15px;">
                                <?php if ($this->agent->is_mobile()) : ?>
                                    <div class="col-sm-12 col-xs-12">
                                        <img src="<?= base_url('assets/user/images/kategori/splus-pvc-foam-board/cab-bath-m.jpg') ?>" style="padding-top:10px;">
                                    </div>
                                <?php else : ?>
                                    <div class="col-md-12">
                                        <img src="<?= base_url('assets/user/images/kategori/splus-pvc-foam-board/cab-bath.jpg') ?>" style="padding-top:10px;">
                                    </div>
                                <?php endif; ?>
                            </div>
                            <div class="row">
                                <h3 style="padding-left: 15px;"><b>Cubicle Toilet</b></h3>
                                <hr style="width: 22%; border: 0.5px solid #e24545; margin-left:15px;">
                                <?php if ($this->agent->is_mobile()) : ?>
                                    <div class="col-sm-12 col-xs-12">
                                        <img src="<?= base_url('assets/user/images/kategori/splus-pvc-foam-board/cub-toi-m.jpg') ?>" style="padding-top:10px;">
                                    </div>
                                <?php else : ?>
                                    <div class="col-md-12">
                                        <img src="<?= base_url('assets/user/images/kategori/splus-pvc-foam-board/cub-toi.jpg') ?>" style="padding-top:10px;">
                                    </div>
                                <?php endif; ?>
                            </div>
                            <br> -->

                            <div class="row pt-40">
                                <div class="col-md-12 col-sm-6 col-xs-12">
                                    <center><a href="<?= base_url('v_kategori') ?>" class="ps-btn h-m-body">Kembali</a></center>
                                </div>

                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </div>
    </div>

    <?php $this->load->view('user/footer') ?>