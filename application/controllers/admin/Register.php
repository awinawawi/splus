...
class Register extends CI_Controller
{
	...
	{
		$config['upload_path'] = './assets/user/images/admin'; //path folder
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				$config['source_image'] = './assets/user/images/admin/' . $gbr['file_name'];
				$config['create_thumb'] = FALSE;
				$config['maintain_ratio'] = FALSE;
				$config['quality'] = '60%';
				$config['width'] = 300;
				$config['height'] = 300;
				$config['new_image'] = './assets/user/images/admin/' . $gbr['file_name'];
	...
	function simpan_pengguna_newup()
	{
		$config['upload_path'] = './assets/user/images/admin'; //path folder
		$config['allowed_types'] = 'gif|jpg|png|jpeg|bmp'; //type yang dapat diakses bisa anda sesuaikan
		$config['encrypt_name'] = TRUE; //nama yang terupload nantinya
	...
	function update_pengguna()
	{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				$nohp = $this->input->post('xkontak');
				$level = $this->input->post('xlevel');
				$old_image = $this->db->get_where('tbl_pengguna', ['pengguna_id' => $kode])->row();
	...
	function hapus_pengguna()
	{
		...
		$q = $data->row_array();
		$p = $q['pengguna_photo'];
		$path = base_url() . 'assets/user/images/admin/' . $p;
		delete_files($path);
