...
class M_pengunjung extends CI_Model{
    ...
    {
        $this->db->select('COUNT(pengunjung_id) as visitor , DATE_FORMAT(pengunjung_tanggal,"%d") as tanggal');
        $this->db->group_by('tanggal');
        $this->db->order_by('tanggal','ASC');
        $this->db->where('MONTH(pengunjung_tanggal)=MONTH(NOW())');
        return $this->db->get('tbl_pengunjung')->result();
    }
