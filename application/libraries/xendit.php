...
class Xendit {
  function __construct () {
    ...
    $this->secret_api_key = 'xnd_development_AcpRdEfEPiiL40yE0G6cWF3njdheyKEWiZhkTyB6Fb7IaZQDZ5qDxFf6FmwCPpt';
  }
  ...
  function createCallbackVirtualAccount ($external_id, $bank_code, $name, $virtual_account_number = null) {
    ...
    function createCallbackVirtualAccountSuggestAmount($id,$amount) {
        $curl = curl_init();

        $headers = array();
        $headers[] = 'Content-Type: application/json';

        $end_point = $this->server_domain.'/callback_virtual_accounts'.'/'.$id;
